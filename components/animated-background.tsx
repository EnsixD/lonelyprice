...
export function AnimatedBackground() {
  ...
  useEffect(() => {
      ...
      for (let x = 0; x <= canvas.width; x += gridSize) {
        for (let y = 0; y <= canvas.height; y += gridSize) {
          if (x < canvas.width) {
            ...
            ctx.lineTo(
              x + gridSize + Math.sin((y + gridSize) * 0.01 + time * 2 + wavePhase1) * waveAmplitude,
              y + gridSize,
            )
            ctx.stroke()
          }
          ...
          if (y < canvas.height) {
            ...
            ctx.lineTo(
              x + gridSize,
              y + gridSize + Math.cos((x + gridSize) * 0.01 + time * 1.5 + wavePhase2) * waveAmplitude,
            )
            ctx.stroke()
          }
        }
    ...
    const animate = () => {
      ...
      bgGradient1.addColorStop(0, `rgba(40, 40, 40, ${0.05 + Math.sin(time) * 0.02})`)
      bgGradient1.addColorStop(0.5, `rgba(20, 20, 20, ${0.03 + Math.cos(time * 1.3) * 0.01})`)
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let x = 0; x < canvas.width; x += 2) {
          const y =
            canvas.height * (0.3 + i * 0.2) + Math.sin(x * frequency + time * speed + (i * Math.PI) / 2) * amplitude
      ...
      particles.forEach((particleA, indexA) => {
        particles.slice(indexA + 1).forEach((particleB) => {
          ...
          if (distance < 120) {
            ...
            lineGradient.addColorStop(0.5, `rgba(200, 200, 200, ${opacity * 0.7})`)
            lineGradient.addColorStop(1, `rgba(150, 150, 150, ${opacity * 0.4})`)
